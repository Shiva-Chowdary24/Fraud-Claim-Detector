...
function Predict() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // REDIRECT INSTEAD OF SHOWING RESULT HERE
      // Passing res.data through the state to the next page
      navigate("/admin/prediction-results", { state: { result: res.data } });

    } catch (error) {
      const msg = error?.response?.data?.detail || error?.message || "Submission failed";
      setErrorMsg(msg);
    } finally {
      setSubmitting(false);
    }
