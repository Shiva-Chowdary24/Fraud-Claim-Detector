...
function CustPolicyHistory() {
    ...
    const fetchHistory = async () => {
      try {
        const res = await axios.get(
          `http://127.0.0.1:8000/customer/full-history?customer_id=${customerId}`
        );
        setHistory(res.data);
      } catch (err) {
  ...
  // ✅ SAFE DATE FORMATTER (NO MORE INVALID DATE)
            ...
            {loading ? (
              ...
              <div className="p-20 text-center border border-dashed border-slate-800 rounded-3xl">
                <p className="text-slate-500">
                  No records found for ID #{customerId}
                </p>
              </div>
            ) : (
              <div className="bg-[#111e32] border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
                <table className="w-full text-left">
                  <thead className="bg-slate-800/50">
                    <tr>
                      <th className="p-5 text-xs uppercase text-slate-400">
                        Policy
                      </th>
                      <th className="p-5 text-xs uppercase text-slate-400">
                        Date & Time
                  ...
                  <tbody>
                    {history.map((item) => (
                      ...
                      >
                        ...
                        <td className="p-5">
                          <span
                            className={`px-3 py-1 rounded-full text-[10px] font-bold border flex items-center gap-2 w-fit ${
                              item.status === "Active"
