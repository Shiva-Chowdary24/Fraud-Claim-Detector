...
import { Plus, ShieldPlus, Trash2, Edit, CheckCircle2, X, Landmark } from "lucide-react"; // Added Landmark icon
...
function IssuePolicies() {
  ...
  // ✅ Added total_claim_amount to State
  const [newPolicy, setNewPolicy] = useState({
    plan_name: "",
    premium_amount: "",
    total_claim_amount: "", // New Field
    tenure: "",
    description: "",
    benefits: "" 
  });
  ...
  const handleAddPolicy = async (e) => {
    e.preventDefault();
    try {
      // ✅ Convert total_claim_amount to Number in the payload
      const payload = { 
        ...newPolicy, 
        premium_amount: Number(newPolicy.premium_amount), 
        total_claim_amount: Number(newPolicy.total_claim_amount), // New Field
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex justify-between items-center mb-10">
          <h2 className="text-3xl font-extrabold text-white tracking-tight text-left">POLICY CATALOG</h2>
          <button 
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-8 py-3 rounded-2xl font-bold transition-all shadow-lg active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {policies.map((p) => (
            <div key={p.id} className="bg-[#111e32]/80 backdrop-blur-sm border border-slate-800 p-6 rounded-3xl shadow-2xl flex flex-col h-full hover:border-blue-500/50 transition-colors text-left">
              ...
              {/* ✅ Display Total Claim Amount Badge */}
              ...
              <div className="flex justify-between items-center border-t border-slate-800/50 pt-5 mt-auto">
                <div className="flex flex-col">
                  ...
                  <span className="text-blue-400 text-lg font-black">${p.premium_amount}<span className="text-xs text-slate-500 font-normal">/yr</span></span>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex justify-center items-center z-50 p-4">
          <form 
            ...
            className="bg-[#0a1628] border border-slate-800 p-10 rounded-[2.5rem] w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto text-left"
          >
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-3xl font-bold text-white">Create Policy</h3>
                <p className="text-slate-400 text-sm mt-1">Set coverage limits and pricing for this plan.</p>
              </div>
            ...
            <div className="space-y-6">
              ...
              {/* ✅ Added Total Claim Amount Input */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase ml-1">Total Claimable Amount ($)</label>
                <input 
                  className="w-full bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-2xl text-emerald-400 font-bold outline-none focus:ring-2 focus:ring-emerald-500 transition-all" 
                  type="number" 
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase ml-1">Yearly Premium ($)</label>
                  <input 
                    className="w-full bg-[#1e293b]/50 border border-slate-700 p-4 rounded-2xl text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all" 
                    type="number" 
                    placeholder="1500" 
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  className="w-full bg-[#1e293b]/50 border border-slate-700 p-4 rounded-2xl text-white outline-none focus:ring-2 focus:ring-blue-500 h-20 resize-none transition-all" 
                  placeholder="Summarize the coverage..." 
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  className="w-full bg-[#1e293b]/50 border border-slate-700 p-4 rounded-2xl text-white outline-none focus:ring-2 focus:ring-blue-500 h-24 resize-none transition-all" 
                  placeholder="Free Checkups, Global Support" 
            ...
            <div className="flex gap-6 mt-10">
              ...
              <button 
                type="submit" 
                className="flex-1 bg-blue-600 py-4 rounded-2xl text-white font-bold hover:bg-blue-500 shadow-xl active:scale-95 transition-all"
              >
                Publish Policy
              </button>
            </div>
