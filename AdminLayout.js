...
function AdminLayout({ children }) {
  ...
  const fetchAdminNotifs = async () => {
    ...
    try {
      ...
      // Fetching notifications specifically for the Admin

      const res = await API.get("/notifications/get/ADMIN");
  ...
  useEffect(() => {
    ...
    // Poll every 10 seconds for new claim or policy requests

    const interval = setInterval(fetchAdminNotifs, 5000);
...
<span className="tracking-tight text-white">
            ...
            Insurance Admin <span className="text-emerald-400">Portal</span>
</span>
</h1>
...
<NavItem to="/admin/manage" icon={<UserMinus size={20}/>} label="Manage Dealers" open={open} />
<NavItem to="/admin/delete" icon={<UserMinus size={20}/>} label="Delete Dealer" open={open} />
<NavItem to="/admin/logs" icon={<ScrollText size={20}/>} label="Fraud Logs" open={open} />

            {/* Sidebar link for Policy Requests - Notification Badge Removed */}
<Link 
